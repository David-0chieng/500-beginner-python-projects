...
- **Cryptographically secure** — uses Python's `secrets` module instead of `random` for secure randomness
- **Strength rating** — displays an entropy-based strength label (very weak to very strong) after generation
- **Safe error handling** — invalid input exits cleanly with a clear message instead of crashing
...
```
No external dependencies are required — the program uses only Python's standard library (`secrets`, `string`, `math`, and `sys`).
...
4. Your generated password is displayed along with its strength rating — store it somewhere safe!
